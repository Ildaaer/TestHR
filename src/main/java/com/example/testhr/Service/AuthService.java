...
public class AuthService {
    ...
    public ResponseEntity<?> signup(@RequestBody RegistrationUserDTO registrationUserDTO, @RequestParam("role") String roleName){
        ...
        if (userService.findUserByName(registrationUserDTO.getUsername()) != null){
            return new ResponseEntity<>(new AppError(HttpStatus.BAD_REQUEST.value(), "Пользователь с указанным именем уже существует"), HttpStatus.BAD_REQUEST);
        }
        Role role = roleService.getRoleByName(roleName);
        User user = userService.createNewUser(registrationUserDTO, role);
        return ResponseEntity.ok( new UserDTO(user.getId(), user.getPassword(), user.getEmail()));
    }
}
